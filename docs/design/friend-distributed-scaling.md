...
### 13.3 不变量对照(迁移后仍须满足 CLAUDE.md §9)
...
- [ ] 认可"现阶段保持单 MySQL,不提前分片"(避免过度工程)。
- [ ] 认可未来扩容形态 = **request 单点 CAS + Kafka 异步幂等建边 + 软上限**(本文 §5)。
- [ ] 认可 `maxFriends` 在分片后降级为**软约束**(但注意 §10:软上限只在 §5 路必须,走 TiDB 可保强一致)。
- [ ] 扩容存储路线:**(A) TiDB 过渡(§8,代码改动最小,保强一致)** / **(B) 分片 MySQL + dtm 编排异步建边(§5/§11)** / **(C) 其他分布式 ACID 库(§11.1)**。
- [ ] 认可主存用**分片库 / TiDB**,Redis 仅作热点缓存(不当好友图主存)。

拍板后:更新 [pandora-arch.md](./pandora-arch.md) §11 决策记录,并在 [PROGRESS.md](../../PROGRESS.md) 追加一条。
